...
class RyzenAIAttention(torch.nn.Module):
    ...
    def merge_qkv_w4abf16(self, q, k, v):
        ...
        if isinstance(q, WQLinear):  # AWQ
        ...
        elif isinstance(q, qlinear.QLinearPerGrp):  # PerGrp: QKV as QLinearPerGrp
            ...
            if q.bias is not None:
                self.qkv_proj.bias = torch.cat((q.bias, k.bias, v.bias), dim=-1)
            else:
                self.qkv_proj.bias = None
        else:  # unquantized QLinearPerGrp
            self.qkv_proj = qlinear.QLinearPerGrp(
                self.in_features,
                self.out_features,
                self.has_bias,
    ...
    def matmul_qkv_w4abf16(self, hidden_states):
        ...
        if seq_len == 1:
            if self.gemm_torch == 1:
                self.qkv_proj.aiegemm.execute_aie(
                    hidden_states.view(torch.bfloat16),
                    self.qkv_states_torch.view(torch.bfloat16),
                    self.qkv_proj.wts_index,
                )
            else:
    ...
    def apply_rotary_pos_emb_token_fast(self, q, k, position_ids, dtype=None):
        if dtype is None:
            dtype = q.dtype
        cos, sin = (
            self.rotary_emb.cos_cached[position_ids].to(dtype),
            self.rotary_emb.sin_cached[position_ids].to(dtype),
        )
