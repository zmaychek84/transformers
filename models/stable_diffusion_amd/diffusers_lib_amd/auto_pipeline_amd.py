...
from diffusers.pipelines.latent_consistency_models import (
...
from diffusers.pipelines.wuerstchen import (
    WuerstchenCombinedPipeline,
    WuerstchenDecoderPipeline,
)
from diffusers.utils import DIFFUSERS_CACHE
from pipeline_stable_diffusion_xl_amd import StableDiffusionXLPipeline
...
class AutoPipelineForText2Image(ConfigMixin):
    ...
    def from_pretrained(cls, pretrained_model_or_path, **kwargs):
        ...
        text_2_image_cls = _get_task_class(
            AUTO_TEXT2IMAGE_PIPELINES_MAPPING, orig_class_name
        )
    ...
    def from_pipe(cls, pipeline, **kwargs):
        ...
        text_2_image_cls = _get_task_class(
            AUTO_TEXT2IMAGE_PIPELINES_MAPPING, original_cls_name
        )
        ...
        additional_pipe_kwargs = [
            ...
            if k.startswith("_")
            and k[1:] in optional_kwargs
            and k[1:] not in passed_pipe_kwargs
        ]
...
class AutoPipelineForImage2Image(ConfigMixin):
    ...
    def from_pretrained(cls, pretrained_model_or_path, **kwargs):
        ...
        image_2_image_cls = _get_task_class(
            AUTO_IMAGE2IMAGE_PIPELINES_MAPPING, orig_class_name
        )
    ...
    def from_pipe(cls, pipeline, **kwargs):
        ...
        # derive the pipeline class to instantiate
        image_2_image_cls = _get_task_class(
            AUTO_IMAGE2IMAGE_PIPELINES_MAPPING, original_cls_name
        )
        ...
        if "controlnet" in kwargs:
            if kwargs["controlnet"] is not None:
                ...
                image_2_image_cls = _get_task_class(
                    AUTO_IMAGE2IMAGE_PIPELINES_MAPPING,
                    image_2_image_cls.__name__.replace(
                        "ControlNetImg2ImgPipeline", "Img2ImgPipeline"
                    ),
                )
...
class AutoPipelineForInpainting(ConfigMixin):
    ...
    def from_pretrained(cls, pretrained_model_or_path, **kwargs):
        ...
        inpainting_cls = _get_task_class(
            AUTO_INPAINT_PIPELINES_MAPPING, orig_class_name
        )
    ...
    def from_pipe(cls, pipeline, **kwargs):
        ...
        # derive the pipeline class to instantiate
        inpainting_cls = _get_task_class(
            AUTO_INPAINT_PIPELINES_MAPPING, original_cls_name
        )
        ...
        if "controlnet" in kwargs:
            if kwargs["controlnet"] is not None:
                ...
                inpainting_cls = _get_task_class(
                    AUTO_INPAINT_PIPELINES_MAPPING,
                    inpainting_cls.__name__.replace(
                        "ControlNetInpaintPipeline", "InpaintPipeline"
                    ),
                )
