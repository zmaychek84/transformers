...
def run_unet(pipe, model_id=""):
    """
    ***** pipe.unet inputs *****
    latent_model_input.shape=torch.Size([1, 4, 64, 64]) latent_model_input.dtype=torch.float32
    ...
    ***** pipe.unet outputs *****
    noise_pred.shape=torch.Size([1, 4, 64, 64]) noise_pred.dtype=torch.float32
    """
    latent_model_input = torch.rand((1, 4, 64, 64)).to(torch.float32)
    t = torch.tensor(1).to(torch.int64)
    prompt_embeds = torch.rand((1, 77, 2048)).to(torch.float32)
    timestep_cond = None
    cross_attention_kwargs = None
    added_cond_kwargs = {
        "text_embeds": torch.rand((1, 1280)).to(torch.float32),
        "time_ids": torch.rand((1, 6)).to(torch.float32),
    }
...
if __name__ == "__main__":
    ...
    # image = pipe(prompt, num_inference_steps=1).images[0]
    # image.save(f"profile.png")
    # count_gops(pipe, {'prompt':prompt, 'num_inference_steps':1, 'height':512, 'width':512, 'guidance_scale':0.0})
    run_unet(pipe, model_id="sdxl")
