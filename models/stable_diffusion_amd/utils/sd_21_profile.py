...
def run_unet(pipe, model_id=""):
    """
    ***** pipe.unet inputs *****
    latent_model_input.shape=torch.Size([2, 4, 96, 96]) latent_model_input.dtype=torch.float32
    t=tensor(1) t.dtype=torch.int64
    prompt_embeds.shape=torch.Size([2, 77, 1024]) prompt_embeds.dtype=torch.float32
    timestep_cond=None
    cross_attention_kwargs=None
    added_cond_kwargs=None
    ***** pipe.unet outputs *****
    noise_pred.shape=torch.Size([2, 4, 96, 96]) noise_pred.dtype=torch.float32
    """
    # for image size 600x600
    latent_model_input = torch.rand((2, 4, 75, 75)).to(torch.float32)
    t = torch.tensor(1).to(torch.int64)
    prompt_embeds = torch.rand((2, 77, 1024)).to(torch.float32)
    timestep_cond = None
    cross_attention_kwargs = None
    added_cond_kwargs = None
    ...
    # OpsProfile.fxtrace(pipe.unet, (latent_model_input, t, prompt_embeds))
...
def run_decoder(pipe, model_id=""):
    ...
    ***** pipe.vae.decode inputs *****
    del1.shape=torch.Size([1, 4, 96, 96]) del1.dtype=torch.float32
    generator=None
    ***** pipe.vae.decode outputs *****
    image.shape=torch.Size([1, 3, 768, 768]) image.dtype=torch.float32
    """
    latents = torch.rand((1, 4, 96, 96)).to(torch.float32)
    generator = None
...
if __name__ == "__main__":
    ...
    # count_gops(pipe, {'prompt':prompt, 'num_inference_steps':20, 'height':600, 'width':600})
    run_unet(pipe, model_id="sd2_1")
    exit()

    # Analyze SD2.1
    for func in [run_clip, run_unet, run_decoder]:
        # run_safety_checker
        func(pipe, model_id="sd2_1")
