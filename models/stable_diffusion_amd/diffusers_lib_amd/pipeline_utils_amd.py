...
def is_safetensors_compatible(filenames, variant=None, passed_components=None) -> bool:
    ...
    for filename in filenames:
        ...
        if (
            ...
            and filename.split("/")[0] in passed_components
        ):
            continue
...
def variant_compatible_siblings(
    ...
    if variant is not None:
        variant_weights = {
            f for f in filenames if variant_file_re.match(f.split("/")[-1]) is not None
        }
        variant_indexes = {
            f for f in filenames if variant_index_re.match(f.split("/")[-1]) is not None
        }
    ...
    non_variant_weights = {
        f for f in filenames if non_variant_file_re.match(f.split("/")[-1]) is not None
    }
    non_variant_indexes = {
        f for f in filenames if non_variant_index_re.match(f.split("/")[-1]) is not None
    }
    ...
    def convert_to_variant(filename):
        if "index" in filename:
            ...
            re.compile(f"^(.*?){transformers_index_format}").match(filename) is not None
        ):
...
def maybe_raise_or_warn(
    ...
    if not is_pipeline_module:
        ...
        if not issubclass(model_cls, expected_class_obj):
            raise ValueError(
                f"{passed_class_obj[name]} is of type: {model_cls}, but should be"
...
def get_class_obj_and_candidates(
    ...
    if class_name == "UNet2DConditionModel":
        ...
        component_folder = dir_path + "\\stable_diffusion_amd"
        if pipeline_class_name == "StableDiffusionPipeline":
            library_name = "unet_2d_condition_amd_sd"
        else:
            library_name = "unet_2d_condition_amd_sdxl"
        # pdb.set_trace()
...
class DiffusionPipeline(ConfigMixin, PushToHubMixin):
    ...
    def register_modules(self, **kwargs):
        ...
        for name, module in kwargs.items():
            ...
            if module is None:
                ...
                pipeline_dir = (
                    module_path_items[-2] if len(module_path_items) > 2 else None
                )
    ...
    def to(self, *args, **kwargs):
        ...
        pipeline_is_offloaded = any(
            module_is_offloaded(module) for _, module in self.components.items()
        )
    ...
    def download(cls, pretrained_model_name, **kwargs) -> Union[str, os.PathLike]:
        ...
        if not local_files_only:
            ...
            expected_files = [
                f for f in filenames if not any(p.match(f) for p in re_ignore_pattern)
            ]
            expected_files = [
                f for f in expected_files if any(p.match(f) for p in re_allow_pattern)
            ]
    ...
    def set_attention_slice(self, slice_size: Optional[int]):
        ...
        modules = [
            m
            for m in modules
            if isinstance(m, torch.nn.Module) and hasattr(m, "set_attention_slice")
        ]
