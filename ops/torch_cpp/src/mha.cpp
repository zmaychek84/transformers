...
torch::Tensor cpu::mha::mha_tensorized(torch::Tensor query_states,
  ...
  auto res = torch::matmul(query_states, key_states.transpose(2, 3));
  res = res * 0.088388347f;
  res = res + attention_mask;
  res = torch::nn::functional::softmax(res.to(torch::kFloat), -1)
            .to(torch::kBFloat16);
...
torch::Tensor cpu::mha::mha_flat_multithread(torch::Tensor query_states,
  ...
  int embed_size = query_states.sizes()[2];
